...
def local_refine(params, t, y, max_iters=10):
    try:
...
# =========================
# 6. WEIBULL PARAMETER ESTIMATION
# =========================
...
# =========================
# 7. ADAPTIVE SA WITH WEIBULL ACCEPTANCE
# =========================
def solve_lppl_jls(t, y, iterations=4000, restart_after=800, warmup=100,
                  T0=10.0, alpha=0.995):
    ...
    for it in range(iterations):
        ...
        if delta < 0:
            ...
            if it < warmup:
                ...
                if cat == "near":
                    near_errors.append(delta)
                    if len(near_errors) > 500: near_errors = near_errors[-500:]
                else:
                    distant_errors.append(delta)
                    if len(distant_errors) > 500: distant_errors = distant_errors[-500:]
        ...
        if no_improve > restart_after:
            print(f"Restart at iteration {it} (no improvement)")
...
# 8. RUN ESTIMATION
# =========================
best_nonlin, best_rss, hist = solve_lppl_jls(t, y, iterations=4000, restart_after=800, warmup=100)
...
print("\n" + "="*60)
print("JLS / LPPL ESTIMATION RESULTS (Filimonov-Sornette linearization)")
...
# =========================
# 9. PLOT WITH DATES ON X-AXIS (FIXED)
# =========================
# Generate fitted curve using the original time array (days) then convert to dates
...
# Convert fitted days to actual dates
fit_dates = dates[0] + pd.to_timedelta(t_fit, unit='D')

# Subplot 1: Log price with dates
...
# Subplot 2: Original price with dates
plt.subplot(1,2,2)
...
# Convergence plot (iterations, unchanged)
plt.figure(figsize=(10,4))
plt.plot(hist)
plt.title('Convergence of RSS (SA with Weibull adaptive acceptance)')
